...
export const SpellListing = (props) => {
  ...
  const displayList = Object.values(spellObject)
    ...
    .map((item, index) =>  <div key={index} className="form-check col-4">
    <input
...
export const SpellsMain = (props) => {
  ...
  return (
    <div>
      <p>
        Available Spells: {level0 && `Level 0: ${level0}`}{" "}
      ...
      <SpellListing level0={level0} setLevel0={setLevel0} level1={level1} setLevel1={setLevel1} selectedClass={props.selectedClass} level={props.level} />
      </div>
    </div>
  );
};
